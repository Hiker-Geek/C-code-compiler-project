...
/*
 *PRE: nothing
 *POST: a temporary label for the symbol table to use
 * */
char * CreateTemp()
{    char hold[100];
     char *s;
...
/*
 *Pre: pointer to char *name, A_OPERATORS Type, and 4 int types 
 *Post: create new item in symbol table
 * */
...

{
  struct SymbTab * n;
    ...
    if(n!=NULL)//we found the symbol in the table already
      {
      printf("\n\tThe name %s exists at level %d already in the symbol table\n\tDuplicate can.t be inserted",name, level);
      return (NULL);
      }
    else//we did not find the symbol in the table so we will make a new one
    {
      struct SymbTab *p;
...
/*
 *Pre: none
 *Post: void, display symbol table
 * */
void Display()
{
   int i;
   ...
   p=first;
   printf("\n\tLABEL\t\tOFFSET\t\tLEVEL\n");
      while (p!=NULL)//p has something init so we will iterate through the table and print everything
      {
         PrintSym(p);
         p=p->next;
      }
...
/*
 *Pre: character array and two integer values
 *Post: pointer to symbol table or null
 * */
struct SymbTab * Search(char name[], int level, int recur)
{
  ...
  /* for each level, try to find our symbol */
   while (level >= 0)
    {
       p=first;
       while (p!=NULL)//while the node is not null, keep checking for our symbol
        {
         if((strcmp(p->name,name)==0) && (p->level == level))//checking for our symbol
           return p;
         p=p->next;
        }
...
int Delete(int level)
{
  ...
  /* cruise through the list */
    ...
    while (p != NULL)//p still has something in it we can check
      {
        /* do we match? */
        if (p->level>= level )
        { /* if it is the first in the list we have to update first, we know this by f being NULL */
           SIZE+=p->mysize;
           if ( f==NULL) first=p->next;//update first
...
/*
 *Pre: ASTnode pointer, ASTnode pointer
 *Post: integer
 * */
int CompareFormals(struct ASTNode *A, struct ASTNode *F) { 
	if(A==NULL && F==NULL) return true;//compare null types to see if the types match. if they do return true
	if(A==NULL || F==NULL) return false;//compare null types to see if the types match. if they don't return false
	if(A->calctype != F->calctype) return false;//if the node types do not match return false
	return (CompareFormals(A->next, F->next));//recursive call
}
